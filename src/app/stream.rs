...
impl FromStr for StreamId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let ms_time = parts[0].parse::<u64>().map_err(|_| AppError::ValueError("Invalid stream ID format: bad ms_time".into()))?;
        let seq_no = parts[1].parse::<u64>().map_err(|_| AppError::ValueError("Invalid stream ID format: bad seq_no".into()))?;
...
impl Stream {
    ...
    fn generate_id(&self, id_spec: &str) -> Result<StreamId, AppError> {
        if id_spec == "*" {
            let mut ms_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u64;
            let mut seq_no = 0;
            ...
            let ms_time = time_part.parse::<u64>().map_err(|_| AppError::ValueError("Invalid stream ID format".into()))?;
            let mut seq_no = 0;
    ...
    pub fn read_from(&self, start_spec: &str) -> Result<Vec<StreamEntry>, AppError> {
        ...
        let range = self.entries.range((Bound::Excluded(start_id), Bound::Unbounded));
        let result = range.map(|(id, fields)| (*id, fields.clone())).collect();
        Ok(result)
    }
    ...
    fn parse_bound(&self, spec: &str, is_start: bool) -> Result<StreamId, AppError> {
        match spec {
            ...
            s => {
                if !s.contains('-') {
                    let ms_time = s.parse::<u64>().map_err(|_| AppError::ValueError("Invalid stream ID format".into()))?;
