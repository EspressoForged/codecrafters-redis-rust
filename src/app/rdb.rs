...
pub fn empty_rdb_bytes() -> Vec<u8> {
    const EMPTY_RDB_HEX: &str = "524544495330303131fa0972656469732d76657205372e322e30fa0a72656469732d62697473c040fe00ff";
        ...
        match opcode {
            ...
            value_type => {
                ...
                if value_type == 0 { 
                    ...
                    warn!("Unsupported RDB value type: {}. Skipping key '{:?}'.", value_type, String::from_utf8_lossy(key));
                    current_expiry.take(); 
                }
...
fn parse_length_encoded(input: &[u8]) -> IResult<&[u8], (usize, bool)> {
    ...
    let len = match length_type >> 6 {
        ...
        0b01 => {
            ...
            return Ok((input, ((((length_type & 0x3F) as usize) << 8) | next_byte as usize, false)));
        },
        0b10 => {
             let (input, bytes) = take(4usize)(input)?;
             return Ok((input, (u32::from_be_bytes(bytes.try_into().unwrap()) as usize, false)));
        },
        0b11 => {
            match length_type {
                0xC0 => 1,
                ...
                0xC2 => 4,
                _ => {
                    warn!("Unsupported RDB special length encoding: {:02x}", length_type);
...
fn convert_systemtime_to_instant(t: SystemTime) -> Instant {
    ...
    if t > now_sys {
        t.duration_since(now_sys).map(|d| now_ins + d).unwrap_or(now_ins)
    } else {
        now_sys.duration_since(t).map(|d| now_ins - d).unwrap_or(now_ins)
    }
}